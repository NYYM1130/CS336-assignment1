...
def train_bpe(
        raw_count: dict[tuple[bytes, ...], int],
        vocab_size: int,
...
def parallel_pretokenize(
    ...
    if num_workers is None:
        num_workers = min(len(chunks), os.cpu_count() or 1)

    global_raw_count: dict[tuple[bytes, ...], int] = {}
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in as_completed(futures):
            local_raw_count = future.result()
            for token_bytes, count in local_raw_count.items():
                global_raw_count[token_bytes] = (
                    global_raw_count.get(token_bytes, 0) + count
                )
    return global_raw_count
...
if __name__ == "__main__":
    ...
    raw_count = parallel_pretokenize(
        input_path=input_path,
